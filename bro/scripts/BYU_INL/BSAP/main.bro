...
export {
	...
	global log_bsap: event(rec: Info);
}
...
event bro_init() &priority=5
	...
	Log::create_stream(BSAP::LOG, [$columns=Info, $ev=log_bsap, $path="bsap"]);
		# Settings for Port-Based DPD (which we aren't using right now)
...
event poll_event(c: connection, head1: BSAP::Header)
        {
        local info: Info;
        info$ts  = network_time();
        info$uid = c$uid;
...
event ack_event(c: connection, head1: BSAP::Header, data1: count)
        {
        local info: Info;
        info$ts  = network_time();
        info$uid = c$uid;
...
event ackNoData_event(c: connection, head1: BSAP::Header, data1: count)
        {
        local info: Info;
        info$ts  = network_time();
        info$uid = c$uid;
...
event nak_event(c: connection, head1: BSAP::Header, data1: count)
        {
        local info: Info;
        info$ts  = network_time();
        info$uid = c$uid;
...
event upAck_event(c: connection, head1: BSAP::Header)
        {
        local info: Info;
        info$ts  = network_time();
        info$uid = c$uid;
